...
function options = convectiveGersh(Fsxx,Fsxy,Fsyx,Fsyy, options)
  ...
  if (order4 == 0)
    ...
    % 
    Fsxx = spdiags(Fsxx,0,N1,N1);
    Fsxy = spdiags(Fsxy,0,N2,N2);
    Fsyx = spdiags(Fsyx,0,N3,N3);
    Fsyy = spdiags(Fsyy,0,N4,N4);
    
    Cu = Cux*Fsxx*Au_ux + Cuy*Fsxy*Au_uy;
    Cv = Cvx*Fsyx*Av_vx + Cvy*Fsyy*Av_vy; 
    ...
    options.stability.bounds.eb_C = max([max(sum_conv_u) max(sum_conv_v)]);
