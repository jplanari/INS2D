function [conv_bc,conv_linear,conv_quad] = operator_rom_convection_lifting_fun(P,options)
% conv_bc: quadratic in a_bc
% conv_lin: linear in a_bc and a_hom
% conv_quad: quadratic in a_hom
...
for i=1:M
    for j=1:M_inhom
        Conv = K*((I*B(:,i)).*(A*F_inhom(:,j))) + K*((I*F_inhom(:,j)).*(A*B(:,i)));
        % convert third order tensor to a matrix via the following indexing:
...
for i=1:M_inhom
    for j=1:M_inhom
        Conv = K*((I*F_inhom(:,i)).*(A*F_inhom(:,j)));
        % convert third order tensor to a matrix via the following indexing:
...
for i=1:M_inhom
    for j=1:M_bc
        Conv = K*((I*F_inhom(:,i)).*y_A(:,j)) + K*((y_I(:,j)).*(A*F_inhom(:,i)));
        % convert third order tensor to a matrix via the following indexing:
...
conv_quad = C_hom;
conv_linear = C_hom_inhom + C_hom_bc;
conv_bc = C_bc + C_inhom + C_inhom_bc;
