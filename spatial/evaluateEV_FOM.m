function options = evaluateEV_FOM(C, options)
    ...
    ev_C = [eigs(Ccu); eigs(Ccv)];
    ev_D = [eigs(Dcu); eigs(Dcv)];
    ev_CD = [eigs(-Ccu+Dcu); eigs(-Ccv+Dcv)];
...
function [] = plotEV(options)
    ev_C = options.stability.ev_C;
    ev_D = options.stability.ev_D;
    ev_CD = options.stability.ev_CD;
    figure;
    hold on
    grid on
    ...
    legend({'Convective','Diffusive','-C+D'},'interpreter','latex')
